...
- generated artifact fallback descriptions can optionally be upgraded with compact batched LLM metadata behind the `artifact_metadata_*` LLM config knobs, so wrappers can experiment with richer generated-code interpretation without enabling full per-chunk descriptions
